  ...
  return (
    <div className="glass-panel rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-300 border border-border/80">
      <div className="flex items-center justify-between mb-6 border-b border-border/40 pb-3">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-foreground font-outfit">Recent Activity Log</h3>
      </div>
        ...
        {items.map((item) => (
          <div key={item.id} className="relative flex items-center justify-between gap-4 group">
            ...
            <div className="flex-grow">
              <p className="text-xs font-semibold text-foreground/90 transition-colors group-hover:text-cyan-400 font-outfit">{item.title}</p>
              <p className="text-[9px] text-muted-foreground mt-0.5 font-orbitron">
            ...
            <span
              className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border tracking-wider font-outfit shrink-0 ${getBg(
                item.type
              )}`}
            >
              {item.type}
