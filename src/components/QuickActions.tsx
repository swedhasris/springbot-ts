  ...
  return (
    <div className="glass-panel rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-300 border border-border/80">
      <div className="flex items-center justify-between mb-4 border-b border-border/40 pb-3">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-foreground font-outfit">Quick Shortcuts</h3>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {actions.map((act) => (
          ...
          >
            ...
            <div>
              <p className="text-xs font-black leading-tight font-outfit">{act.title}</p>
              <p className="text-[9px] text-muted-foreground mt-1 line-clamp-1 leading-normal font-outfit">{act.description}</p>
            </div>
          </Link>
        ))}
      </div>
