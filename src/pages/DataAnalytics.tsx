...
function SummaryCard({ config, count, previousCount }: { config: typeof CARD_CONFIGS[0]; count: number; previousCount: number }) {
...
export function DataAnalytics() {
  ...
  const filteredTickets = useMemo(() => {
    return roleFilteredTickets.filter((t) => {
      ...
      // Category filter
      if (categoryFilter !== "all" && normalizeText(t.category) !== normalizeText(categoryFilter)) return false;
  ...
  const filterOptions = useMemo(() => {
    ...
    const categories = [...new Set(roleFilteredTickets.map((t) => t.category).filter(Boolean))].sort();
